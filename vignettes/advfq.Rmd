...
With the now-derived transformed parameters (identifiable through `PARCAT2 == "Transformed - Original Items"`), we have every question measured on the same scale and can now derive the composite parameters. These are essentially just means across categories of questions, with the category being determined by the contents of `PARCAT4`. The VFQ questionnaire dictates that for each category, two different composite parameters should be derived: one including just the base items of the questionnaire (`PARCAT5 == "Base Item"`) and the other one also including any optional items. As such, below we derive the composite parameters in two stages, first constructing `advfq_qsb` for the base item composite means, and then `advfq_qso` for the composite means, including all items. Note that the `dataset` argument is not passed to `derive_summary_records()`, meaning the output dataset contains only the new composite parameters. This is so that we can then use `derive_vars_merged_lookup()` to add on the `PARCATx` and `PARAM` variables for these new records. At the end, we append the new records to the existing `advfq` using `bind_rows()`.
...
Here's what the base item composite records look like for the "Near Activities" and "Distance Activities" categories for one patient's first two visits:
