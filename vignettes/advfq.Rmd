...
Here's what the data for the first three questions looks like for one patient only:

```{r, eval=TRUE, echo=FALSE}
dataset_vignette(
  advfq %>% 
    filter(USUBJID %in% c("01-701-1034"), PARAMCD %in% c("VFQ101", "VFQ102", "VFQ103")) %>% 
    arrange(USUBJID, AVISITN, PARAMCD),
  display_vars = exprs(USUBJID, QSTESTCD, AVISIT, PARAMCD, AVAL, AVALC)
)
```
## Example Script {#example}
